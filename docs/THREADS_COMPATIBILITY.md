...
| Core Thread views, graph projection semantics, health, roles, edge direction, waves, and frontier | Stable semantic contract | Every adapter receives the same taskflow-owned values. Member/external-gate roles, prerequisite-to-dependent edges, graph/projection health, sound completion, and frontier eligibility cannot be reinterpreted by a presentation adapter. |

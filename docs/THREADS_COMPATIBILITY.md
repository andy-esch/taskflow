...
| `--json` envelopes, projections, field meanings, vocabularies, and error codes | Stable revisioned machine contract | `schema_version` is one monotonic contract revision, independent of the binary version and explicitly not SemVer ([ADR-0008](../planning/adrs/0008-use-monotonic-revisions-for-the-json-machine-contract.md)). Beginning at 1.68 every revision declares `ADDITIVE` or `NOT ADDITIVE`; closed-vocabulary changes are non-additive because exhaustive consumers may switch on them. Consumers inspect the invoked binary's `schema --json`, tolerate unknown additive object fields, and compare exact revisions rather than major-version ranges. The generated JSON Schema covers typed envelopes; caller-selected projection rows remain revisioned but dynamic. |

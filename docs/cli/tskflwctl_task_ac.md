...
A `tracked` criterion additionally REQUIRES --to <task>, the destination it was handed to. The reference is resolved to that task's stable id at write time and the id is what gets recorded, so a later rename of the destination cannot break the pointer — the same reason a finding's `tracked` status names `tracked by <task-id>`. --reason keeps the prose.
...
```
  ...
  -h, --help              help for ac
      ...
      --to task           the destination task a --tracked criterion was handed to (resolved to its stable id)

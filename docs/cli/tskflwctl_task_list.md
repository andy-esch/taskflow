...
```
  ...
  -c, --columns strings   select columns for -o table/csv/json, comma-separated (-o table when no format is pinned); available: slug,status,tier,priority,epic,updated,description,revisit_at

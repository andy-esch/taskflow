...
var entities = []Descriptor{
	{
		...
		Conventions: []string{
			...
			"depends_on is a sorted set of stable task IDs owned by the repository-global DAG; use `task depend add/remove` once available — generic `task set` and `task edit` cannot change it.",

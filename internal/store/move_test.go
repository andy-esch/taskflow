...
func TestFS_Move(t *testing.T) {
	...
	task, err := NewFS(root).Move("alpha", domain.StatusInProgress, now)
	if err != nil {
		t.Fatal(err)
	}
...
func TestFS_Move_Idempotent(t *testing.T) {
	...
	task, err := NewFS(root).Move("beta", domain.StatusInProgress, time.Now())
	if err != nil {
		t.Fatal(err)
	}
	if task.Status != domain.StatusInProgress {
...
func TestFS_Move_NotFound(t *testing.T) {
	_, err := NewFS(t.TempDir()).Move("nope", domain.StatusCompleted, time.Now())
...
func TestFS_Resolve_Ambiguous(t *testing.T) {
	...
	_, err := NewFS(root).Move("dup", domain.StatusCompleted, time.Now())

...
// linkRe captures BOTH halves of an inline markdown link — `[label](target)` — because
// a rename has to fix the label as well as the target. The label is captured
// non-greedily and forbids `[`/`]` so nested brackets don't swallow neighbouring links.
var linkRe = regexp.MustCompile(`\[([^\[\]]*)\]\(([^)]+)\)`)

// relabel returns the link text to use for a moved target. Links into `planning/` were
// generated by wikimigrate from the filename stem, so a label that still equals the OLD
// stem is a filename echo that a rename makes stale — it would go on displaying
// `2026-06-11-critical-review-and-polish-research` while pointing at the id-led file.
// Rewrite those to the new human slug (id stripped), which is the convention tasks and
// epics already use: `[slug](<id>-<slug>.md)`.
//
// A label the author actually wrote ("the color overhaul doc") is left alone — matching
// the old stem exactly is the whole test, so prose is never touched.
func relabel(label, targetOld, targetNew string) string {
	oldStem := strings.TrimSuffix(path.Base(targetOld), ".md")
	if label != oldStem {
		return label // human-authored prose, or already correct — don't touch
	}
	newStem := strings.TrimSuffix(path.Base(targetNew), ".md")
	if _, slug, ok := splitIDLed(newStem); ok {
		return slug
	}
	return newStem
}

// splitIDLed splits `<id>-<slug>` into its parts (the read side of isIDLed).
func splitIDLed(stem string) (entityID, slug string, ok bool) {
	if !isIDLed(stem) {
		return "", "", false
	}
	return stem[:idpkg.Length], stem[idpkg.Length+1:], true
}

// rewriteLinks repoints every inbound relative-path markdown link across the tree to
// the renamed research files — target AND label — and writes out the
// frontmatter-backfilled content.
func rewriteLinks(root string, moves map[string]string, pre map[string][]byte) ([]fileEdit, error) {
	...
	err := filepath.WalkDir(root, func(p string, d os.DirEntry, err error) error {
		...
		out := linkRe.ReplaceAllFunc(content, func(match []byte) []byte {
			...
			changes++
			return []byte("[" + relabel(label, targetOld, targetNew) + "](" +
				relLink(path.Dir(newRel), targetNew) + anchor + ")")
		})
...
func report(renames []move, skipped []string, edits []fileEdit, created, backfilled int, apply bool) {
	...
	fmt.Printf("  %d inbound link(s) across %d file(s) %s be repointed (target + stale filename label)\n", linkCount, linkFiles, verb)
	fmt.Println("\nRenames:")
	for _, r := range renames {
		fmt.Printf("  %s\n    -> %s\n", r.old, r.new)
	}

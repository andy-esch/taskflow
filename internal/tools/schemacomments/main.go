...
// map is committed + //go:embed-ed by render.JSONSchema. A drift test
// (render.TestSchemaComments_NotStale) fails if this output goes stale.
//
...
const (
	...
	outPath = "internal/cli/render/schema_comments.json"
)

func main() {
	r := new(jsonschema.Reflector)
	// Only the packages the schema references — the envelope types (render) and the

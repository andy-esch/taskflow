...
func TestHelpContextDependent(t *testing.T) {
	list := strings.Join(helpLines(focusList, entityTasks), "\n")
	detail := strings.Join(helpLines(focusDetail, entityTasks), "\n")
...
func TestModel_HelpScrollClampedToVisibleMax(t *testing.T) {
	...
	if maxScroll <= 0 || maxScroll >= len(helpLines(m.focus, m.cur().kind)) {
		t.Fatalf("test needs an overflowing overlay with a real clamp; helpMaxScroll=%d, lines=%d", maxScroll, len(helpLines(m.focus, m.cur().kind)))
	}

	// Press j well past the bottom — it must stop at the visible max.
	for i := 0; i < len(helpLines(m.focus, m.cur().kind))+5; i++ {

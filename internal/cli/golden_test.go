...
func TestValidateMachineGoldenUpdate(t *testing.T) {
	changed := []byte(`{"schema_version":"1.71","new_contract_field":true}`)
	if err := validateMachineGoldenUpdate("schema_json", nil, changed, "1.71"); err == nil ||
		!strings.Contains(err.Error(), "unchanged revision 1.71") {
	...
	wrongRevision := []byte(`{"schema_version":"1.70","new_contract_field":true}`)
	if err := validateMachineGoldenUpdate("schema_json", nil, wrongRevision, "1.69"); err == nil ||
		!strings.Contains(err.Error(), "running contract is 1.71") {
	...
	before := []byte(`{"schema_version":"1.71","template":{"kind":"audit","name":"security","description":"same"},"body":"old"}`)
	bodyChanged := []byte(`{"schema_version":"1.71","template":{"kind":"audit","name":"security","description":"same"},"body":"new"}`)
	if err := validateMachineGoldenUpdate("template_show_security_json", before, bodyChanged, "1.71"); err != nil {
		t.Fatalf("ADR-0008 excludes a body-only Markdown template change from the JSON revision: %v", err)
	}
	metadataChanged := []byte(`{"schema_version":"1.71","template":{"kind":"audit","name":"security","description":"changed"},"body":"new"}`)
	metadataRemoved := []byte(`{"schema_version":"1.71","template":{"name":"security","description":"same"},"body":"new"}`)
	for _, tc := range []struct {
		...
		if err := validateMachineGoldenUpdate(tc.name, before, tc.data, "1.71"); err == nil ||
			!strings.Contains(err.Error(), "unchanged revision 1.71") {

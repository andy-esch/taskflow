...
**Completed 2026-06-28.** The keyMap is now the single source of the key vocabulary: every binding carries key.WithHelp(displayKey, desc) (keys.go). The `?` overlay rows derive from the bindings via `he()` (help.go) — only sub-component keys (list/viewport j/k + paging) and the list`s own `/` filter stay literal. The three repeated detail-footer fragments are factored into `detailFooterBody()`, and the tab/dashboard footer hints derive their key strings from the bindings via `keyHint`/`keyCombo` (model.go). Guards: TestKeyMapBindingsSelfDescribe (every binding self-describes) + TestHelpAndFooterDeriveFromKeymap. Non-breaking TUI render (the `o/O` and `s/S` rows de-combine into one per binding — clearer; `l/⏎` spacing normalized). The zoomed-footer `: cmd` accuracy nit is deliberately left for a separate UX call, not this DRY pass.

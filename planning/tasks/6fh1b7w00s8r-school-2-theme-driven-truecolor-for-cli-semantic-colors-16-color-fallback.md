...
The CLI Style uses the theme's **Dark** palette (no eager OSC-11 background query, per the latency discipline). So on a *light* terminal with truecolor, semantic colors render the dark-tuned hexes (a contrast concern) — same deferral as the bar gradient. Ties into [validate-and-visually-tune-the-neon-day-light-palette](6fh1b7w02xwd-validate-and-visually-tune-the-neon-day-light-palette.md) and a possible future lazy CLI background detection. Also: the bars still emit truecolor with no 16-color fallback (pre-existing; a separate polish).

## Reference
Supersedes the terminal-native assumption baked into [route-progress-bars-and-the-cli-ansi-map-through-the-palette](6fgq1n003wty-route-progress-bars-and-the-cli-ansi-map-through-the-palette.md) (T3). Design doc: `planning/research/6fgq1n001pwm-color-palette-and-theming-overhaul.md`.

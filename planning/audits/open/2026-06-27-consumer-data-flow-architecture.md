...
#### H2. `core.Service.Summary()` re-reads every audit file from disk a second time  · **Status:** open
...
#### H3. The lifecycle verb→destination vocabulary is duplicated per adapter with no shared registry  · **Status:** open
...
#### M5. `deprecate` confirmation exists only in the TUI; relative revisit dates use wall-clock, not the injected clock  · **Status:** open
...
#### M7. `ListAudits` doesn't validate an unknown bucket — asymmetric with `ListTasks`, a latent web trap  · **Status:** open
